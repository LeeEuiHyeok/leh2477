...
##**1-1 문제상황**
  ...
  3. 유럽은 기차여행이 가능해서 한 달 이상 배낭여행하는 여행객 증가, 나라 내에서 도시 이동시에 동행이 필요 인식
...
#**2-4 우리의 장점**
 1. 유렵여행은 한 나라&도시에 머무는 것이 아니라 여러 나라를 이동하기때문에 채팅기능과 채널 기능을 통해 도시 별로 동행을 구할 수 있는 것이 장점이다.
 2. 범죄 방지 기능을 통해 다른 사용자들에게도 위험인물임을 인지시켜 줄 수 있어 예방하기가 쉬워진다.
 ...
 4 정보 공유 게시판 기능을 통해 여행 일정에 차질이 없게 하며 헛걸음을 하지 않도록 미리 예방해준다. 관광명소 내부 수리와 같은 정보는 여행 중 인 같은 지역에 있는 사람들에게 듣는 것이 가장 빠르고 정확하므로 신뢰성 있는 게시판이 될 수 있다. 
 5. 사진 검색 및 태그 기능을 통해 같은 장소의 사진이지만 사용자마다 올린 사진이 다르기 때문에 다양한 간접경험을 할 수 있다.
  자기가 가고자 하는 장소에 대한 사진 업로드 한 사람에게 연락이 가능하여, 잘 모르는 장소에 대한 정보를 얻을 수 있다.  단순한 소통 어플리케이션이 아닌 사진, 동여상도 공유할 수 있으므로 생생한 여행정보를 공유 할 수 있다.
...
#**6. 고찰 및 성찰**
 ...
 - 한준희 :이번 컴퓨터 시스템 설계 수업을 들으면서 git이라는 협업 프로그램을 사용하면서 상당히 편리하다는 점을 알 수 있었다. 또한               시스템 설계를 하면서 시스템에 대한 전반적인 설계 내용을 이해하는데 큼 도움이 되었다. 처음 시스템 아이디어를 창출하고                  아이디어를 설계하게되면서 많은 부분 부족하다는 생각이 들었다. 설계 하는 부분에서 아이디어 만으로도 가능 하다고 생각했지만,            막상 설계하려하니 전공 적으로 풍부한 지식을 가지고 그 지식을 기반으로 설계를 할수 있구나 라는 생각이들었다. 이전에는 과제를           하게되면 설계를 하기보다는 코딩을 하면서 이것저것 살을 붙이는 방법으로 진행 하였는데 이런 방식의 실계후 코딩으 하게되면               보다 쉽게 코딩작업을 할수 있겠다는 생각이 들었다. 한 학기 동안 수고 많으 셨습니다.
 - 이현수 : 한 학기동안 프로그램을 만들기 직전 단계까지 설계하는 과정과 GitHub을 통해 효율적인 문서작업과 협업을 할 수 있다는걸 배웠다. 먼저 설계하기 전 아이디어를 정하는데 있어서 풍부한 전공지식이 있어야 더욱 수월하다는 것을 느꼈다. 그리고 설계하는 과정에서 많은 것을 배웠으며, 평소에는 코딩만 할 수 있으면 되는 줄 알았는데 수업을 들은 후 설계하는 능력이 뒷받침 되지 않으면 힘들다는 것을 배웠다.이를 통해 다음에 하는 텀프로젝트, 기타 설계과목, 졸업설계를 더욱 쉽게 접근할 수 있을 것 같다. 나아가서 취업을 하고도 설계하는 
과정에서도 도움이 많이 될 것 같다. 
